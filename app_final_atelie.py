...
# --- Parte 1: Ligação ao SUPABASE (PREENCHER!) ---
# NÃO COLOQUE AS SUAS CHAVES AQUI
# O código vai lê-las do "cofre" do Streamlit (st.secrets)
try:
...
# --- Parte 2: Definição das Constantes (Idênticas) ---
...
class Peca:
    """O "molde" para cada peça de cerâmica com as regras 8.2."""
    ...
    def recalcular_custos(self):
        """(RE)Calcula os custos da peça com as NOVAS regras 8.2."""
    ...
    def to_dict(self):
        """Converte o objeto Peca num dicionário para a base de dados."""
        return {
            "id": self.id,
            ...
            "tipo_argila": self.tipo_argila,
            "preco_argila_propria": self.preco_argila_propria,
            "data_registro": self.data_registro,
            "image_path": self.image_path,
            "custo_argila": self.custo_argila,
            "custo_biscoito": self.custo_biscoito,
            "custo_esmalte": self.custo_esmalte,
            "total": self.total
        }

    @classmethod
    def from_dict(cls, data_dict):
        """Cria um objeto Peca a partir de um dicionário (da BD)."""
        peca = cls(
            peca_id=data_dict.get('id'),
            data_producao=data_dict.get('data_producao'),
            nome_pessoa=data_dict.get('nome_pessoa'),
            tipo_peca=data_dict.get('tipo_peca'),
            peso_kg=float(data_dict.get('peso_kg', 0)),
            altura_cm=float(data_dict.get('altura_cm', 0)),
            largura_cm=float(data_dict.get('largura_cm', 0)),
            profundidade_cm=float(data_dict.get('profundidade_cm', 0)),
            tipo_argila=data_dict.get('tipo_argila', 'nenhuma'),
            preco_argila_propria=float(data_dict.get('preco_argila_propria', 0)),
...
# --- Parte 4: Funções de Dados (ATUALIZADA) ---

def carregar_dados():
    """Carrega as peças da base de dados Supabase."""
    try:
        response = supabase.table('pecas').select('*').order('created_at', desc=True).execute()
        dados = response.data
        if dados:
            inventario_objetos = [Peca.from_dict(d) for d in dados]
            return inventario_objetos
    except Exception as e:
        st.error(f"Erro ao carregar dados: {e}")
    return []

def salvar_nova_peca(nova_peca: Peca, uploaded_file):
    """Salva a nova peça e faz o upload da imagem."""
    image_db_path = None
    if uploaded_file is not None:
        try:
            ...
            supabase.storage.from_(NOME_BUCKET_FOTOS).upload(
                ...
                file_options={"content-type": uploaded_file.type, "upsert": "true"} # Upsert=True permite substituir
            )
    ...
    try:
        ...
        if image_db_path: # Tenta apagar a foto se os dados falharem
...
def get_public_url(image_path):
    """Pega a URL pública de uma imagem no Supabase."""
    if not image_path:
        return None
    try:
...
def gerar_relatorio_pdf(lista_de_pecas):
    ...
    # (Esta função é IDÊNTICA à V8.2. Não precisa mudar)
    ...
    pdf.cell(0, 10, 'Relatorio de Producao do Atelie', ln=True, align='C')
    pdf.ln(5)
    ...
    for peca in lista_de_pecas:
        ...
        if image_url:
            try:
                y_antes = pdf.get_y()
                pdf.image(image_url, x=170, y=y_antes, w=30)
    ...
    pdf.cell(0, 5, '--- RESUMO TOTAL ---', ln=True, align='C')
    pdf.set_font('Arial', '', 10)
    pdf.cell(0, 5, f"Total de pecas: {len(lista_de_pecas)}", ln=True)
    pdf.cell(0, 5, f"CUSTO GERAL TOTAL: R$ {custo_geral_total:.2f}", ln=True)
    pdf.ln(5)
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 5, '--- RESUMO POR PESSOA ---', ln=True, align='C')
    pdf.set_font('Arial', '', 10)
...
st.set_page_config(page_title="Gestão Ateliê", layout="wide")
st.title("🏺 Sistema de Gestão de Custos do Ateliê (v8.3)")
...
pagina = st.sidebar.radio("O que você deseja fazer?", 
                         ["Ver Relatório Completo", "Adicionar Nova Peça", "Excluir Peça"]) # <-- NOVA PÁGINA

# --- PÁGINA 1: ADICIONAR NOVA PEÇA (ATUALIZADA) ---
if pagina == "Adicionar Nova Peça":
    ...
    with st.form(key="nova_peca_form", clear_on_submit=True):
        ...
        # --- LÓGICA DE ARGILA CORRIGIDA (NO FORMULÁRIO) ---
        st.subheader("Custos de Material")
        tipo_argila_escolha = st.radio(
            "Qual argila foi usada?",
            ("Argila Própria", f"Argila do Ateliê (R$ {PRECO_ARGILA_ATELIE_KG}/kg)"), # <-- OPÇÃO "Nenhuma" REMOVIDA
            index=0 
        )
        ...
        else: # Só pode ser "Argila do Ateliê"
        ...
        if not nome_pessoa or not tipo_peca or not data_producao or peso_kg == 0:
            ...
            with st.spinner("A criar e salvar a nova peça..."):
                nova_peca = Peca(
                    ...
                    tipo_argila=tipo_argila_final,
                    preco_argila_propria=preco_argila_propria_input
                )
...
# --- PÁGINA 2: VER RELATÓRIO (Idêntica) ---
    ...
    if not inventario:
        ...
        # (Filtros, PDF, Exibição... tudo idêntico à V8.2)
        ...
        if nome_do_pdf:
            try:
                with open(nome_do_pdf, "rb") as f:
                    st.download_button(
                        label="Baixar Relatório em PDF", data=f,
                        file_name=nome_do_pdf, mime="application/pdf"
                    )
            except FileNotFoundError:
                st.error("Erro ao ler o ficheiro PDF gerado.")
...
# --- NOVA PÁGINA 3: EXCLUIR PEÇA ---
    ...
    if not inventario:
        ...
        # Cria um dicionário de {Nome de Exibição: Objeto Peca}
        opcoes_pecas = {
            f"{p.data_producao} - {p.tipo_peca} (por {p.nome_pessoa})": p 
            for p in inventario
        }
        
        # Cria o menu dropdown
        peca_selecionada_nome = st.selectbox(
            "Selecione a peça que deseja excluir:",
            ["Selecione..."] + list(opcoes_pecas.keys())
        )
        
        # Se o usuário selecionou uma peça...
        if peca_selecionada_nome != "Selecione...":
            peca_obj = opcoes_pecas[peca_selecionada_nome]
            
            # Mostra os detalhes para confirmação
            st.subheader("Você selecionou esta peça:")
            image_url = get_public_url(peca_obj.image_path)
            if image_url:
                st.image(image_url, width=200)
            st.write(f"**Tipo:** {peca_obj.tipo_peca}")
            st.write(f"**Pessoa:** {peca_obj.nome_pessoa}")
            st.write(f"**Custo Total:** R$ {peca_obj.total:.2f}")
            
            st.divider()
            
            # Botão de exclusão (vermelho)
            if st.button(f"Confirmar Exclusão Permanente de '{peca_obj.tipo_peca}'", type="primary"):
                with st.spinner("Excluindo peça da base de dados e do armazenamento..."):
                    if excluir_peca_db(peca_obj):
                        ...
                        st.error("Falha ao excluir a peça.")